...
bun_id <- param_map[task_id, stgpr_bundle_id]

measure_name <- "proportion"
type <- "stgpr"
...
bv_id <- map[bundle_id==bun_id, gbd2020_bv]
...
bun_data[age_start == 999, age_start := orig_age_start]
bun_data[age_end == 999, age_end:= orig_age_end]
bun_data[, age_demographer := 0]
bun_data <- bun_data[, -c("orig_age_start", "orig_age_end")]

bun_data <- bun_data[, -c("age_group_id")]

# Adjustments for STGPR models
bun_data[, mean := val]
bun_data[, standard_error := sqrt(variance)]
bun_data[is.na(cases), cases := val * sample_size]
bun_data[, upper := mean + (1.96 * standard_error)]
bun_data[, lower := mean - (1.96 * standard_error)]


#this function checks for any missing values, like standard error, and fills them in. it also
#subsets the data down to just the measure that was supplied as an argument
bun_data <- pull_bundle_data(measure_name = measure_name,
                             bun_id = bun_id,
                             bun_data = bun_data)
...
full_bundle$mean <- NULL
full_bundle[age_start == 0, age_group_id := 2]
full_bundle[age_start >= 0.018 & age_start <= 0.0192, age_group_id := 3]
full_bundle[age_start >= 0.075 & age_start <=  0.077, age_group_id := 388]
full_bundle[age_start >= .49 & age_start <= .52, age_group_id := 389]
full_bundle[age_start >= .98 & age_start <= 1.01, age_group_id := 238]
full_bundle[age_start >= 1.98 & age_start <= 2.01, age_group_id := 34]
full_bundle$unit_value_as_published <- NULL
full_bundle$unit_value_as_published <- 1
full_bundle[val == 1, is_outlier := 1]
full_bundle[sample_size < 30, is_outlier := 1]
full_bundle[is.outlier == 1, is_outlier := 1]
full_bundle$is.outlier <- NULL
