...
assemble_prior<-function(data, rmses, cov_list, data_transform, pred_ages,
  ...
  source(paste0(stgpr_central, "filepath"))
  source(paste0(stgpr_central, "filepath"))
  source("filepath")
  ...
  subnat_locs<-get_location_metadata(101, gbd_round_id=7)$location_id
  ...
  for(sexchar in sex_list){
    ...
    if(plot_mods==T){
      ...
      command<-paste0("qsub -q all.q -l fthread=1 -l h_rt=00:05:00 -l m_mem_free=2G", " -P ", proj, " -N ", paste0("average_", sexchar),
                        " -o /share/temp/sgeoutput/", username, "/output -e filepath", username, "/errors", " -t ", n_jobs,
                        " -tc 500 filepath",
