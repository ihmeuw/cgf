...
test_prior <- function(crosswalk_version_id=NA, path_to_data=NA, decomp_step, cov_list, data_transform,
  ...
  stgpr_central <- "filepath" #path to stgpr root
  output_folder <- paste0("filepath") #path to temporarily save model outputs--this folder gets created and deleted during the function
  ...
  locs <- get_location_metadata(location_set_id=location_set_id, gbd_round_id=7)[, .(location_id, super_region_name, region_name, location_name, level)]
  ...
  for(sexchar in sex_list){
    ...
    for(i in 1:max(seq$id)){
      ...
      command <- paste0("qsub -q all.q -l h_rt=00:15:00 -l m_mem_free=", m_mem_free, "G -l fthread=2 -P ", proj, " -N ", paste0("oos_", sexchar, "_", i),
                        " -o /share/temp/sgeoutput/", username, "/output -e /share/temp/sgeoutput/", username, "/errors", 
                        " /share/code/mnch/st_gpr/linear_oos/shell.sh ",
                        start, " ", end, " ",
