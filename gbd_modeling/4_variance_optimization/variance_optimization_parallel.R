...
loc_metadata <- get_location_metadata(location_set_id = 35, release_id = 9)
...
calc_optim_sd <- function(b,mean,prev_sev,prev_mod,weight_list) {
  ...
  return(as.numeric(((e_prev_sev-prev_sev)^2 + (e_prev_mod-prev_mod)^2)))
...
integrate_draws <- function(loc, sex, age, year, draw, weight_version, var) {
  ...
  M <- as.numeric(mean_val[year_id == year & sex_id == sex & age_group_id == age,
                           paste0("draw_", draw), with=F])
  M <- (M * 10)# should be (m*10)-10, but have to add 10 to get no negative numbers
  sev <- as.numeric(sev_val[year_id == year & sex_id == sex & age_group_id == age,
                            paste0("draw_", draw), with=F])
  mod <- as.numeric(mod_val[year_id == year & sex_id == sex & age_group_id == age,
                            paste0("draw_", draw), with=F])
...
grid <- data.table(expand.grid(loc,sex, age, year, 0:999))
...
lower.sd <- quantile(output$sdev, probs = .025)
upper.sd <- quantile(output$sdev, probs = .975)
