...
min_KS_m2 <- function(allData, weights, initial_condition, all_CDF_Data_Params) { 
  ...
  allweightfits <- lapply(unique(allData$nid_loc_yr_index), function(nid_loc_yr_i){
    ...
    # Requirement of central code that XMAX be set as a global variable
...
# TO-DO: move all data into new folder
...
out_dir = file.path("filepath", me_type, strategy, option)
...
allData <- allData[study.index >49]
length(unique(allData$nid_loc_yr_index))
