...
#Helper functions for weight fitting
...
goodness_of_fit_calc <- function(Data, allData, CDF_Data_Params, weights, option, low_threshold, medium_threshold, high_threshold, low_tail, middle_tail, high_tail, low_weight, medium_weight, high_weight) {
  ...
  if (option == "thresholds_1"){ #old way used in GBD 2019, looking for the biggest difference at all anywhere in the distribution
  ...
  } else if (option == "thresholds_2") { # old way used in GBD 2019, looking for the biggest difference at all anywhere on the NEGATIVE Z score side of the distribution
